...
\begin{document}
    ...
    An interesting property found in~\cite[p.~637]{probIntroRoss} can be used here:

    Our deterministic term is constant and our stochastic term is composed of the random variable $\epsilon$ scaled by a constant.
    $\epsilon \sim N(0,1)$ thus $\operatorname{Var} \!\left[ \epsilon \right]  = 1$

    Using properties of the variance:

    \[\operatorname{Var}\!\left[\ln\!\left(\frac{S_{t+\delta t}}{S_t}\right)\right] = \sigma^2\, \delta t\]
